...
#NUM_ELE = 4
NUM_ELE = env.GlobalSetting.NUM_ELE
...
def elementVoxel(lab, *args):
    ...
    for i in range(len(subsection)):
        if args[0] != 'Wave Trans':
            ax.imshow(subsection[i], cmap=cmaps1[i], alpha=1)
        else:
            cmLimit = max([abs(min(stat)),max(stat)])
            ax.imshow(subsection[i], cmap=cmaps1[i],vmin=-cmLimit/4, vmax=cmLimit/4, alpha=0.5)

    plt.savefig(lab + '-' + args[0] + '-CUTOFF=' +str(CUTOFF) + '-' +str(THETA)+ '-' + str(VOXESCALE) + '.jpg')
